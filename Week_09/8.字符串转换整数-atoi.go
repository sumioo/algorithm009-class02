...
func getState(state string, char byte) string {
	if char == ' ' {
...
func isNegetive(char byte) bool {
	if char == '+' {
		return false
	}
	return true
...
func atoi(str string) int {
	...
	for i := 0; i < len(str); i++ {
		...
		if state == "number" {
			r := int(str[i] - '0')
			if ans > math.MaxInt32/10 || (ans == math.MaxInt32/10 && r > 7) { //负数的值是比正数多一个的，有可能r=8, 此时正数溢出了，但负数没有溢出，但是题目要求溢出后返回对应的最值,所以这样判断也没错。但就没办法判断负数是否真的溢出了
				if isNegetive(sign) {
